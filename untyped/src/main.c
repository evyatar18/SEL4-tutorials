...
int main(int argc, char *argv[]) {
    ...
    for (seL4_CPtr slot = info->untyped.start; slot != info->untyped.end; slot++) {
        seL4_Word idx = slot - info->untyped.start;
        seL4_UntypedDesc *desc = &info->untypedList[idx];
    ...
    seL4_Word total_bytes = 0;
    for (int i = 0; i < sizeof(sizes) / sizeof(sizes[0]); ++i) {
        total_bytes += 1 << sizes[i];
    }
    ...
    // find how many bits we need to represent `total_bytes`
    while (total_bytes > (1 << untyped_size_bits)) {
        ++untyped_size_bits;
    }
    printf("untyped_size_bits=%d\n", untyped_size_bits);
    ...
    error = seL4_Untyped_Retype(child_untyped, // the untyped capability to retype, hope there is enough space (how much space does a TCB take)
                            seL4_TCBObject, // type
                            0,  //size
                            seL4_CapInitThreadCNode, // root
                            0, // node_index
    ...
    ZF_LOGF_IF(error, "Failed revoking child cap");
